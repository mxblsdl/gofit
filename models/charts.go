...
func (data *HeartChartData) GenerateRestingHeartRateChart() string {
	...
	// Prepare resting heart rate data
	restingRates := make([]int, len(data.Series["Heart Rate"]))
	for i, entry := range data.Series["Heart Rate"] {
		restingRates[i] = entry.RestingRate
	}

	// Add resting heart rate series
	line.AddSeries("Resting Heart Rate", generateBarItems(restingRates))
